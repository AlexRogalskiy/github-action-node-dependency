...
const stringWidth = __nccwpck_require__(5537)
...
const stripAnsi = __nccwpck_require__(1411);
const isFullwidthCodePoint = __nccwpck_require__(5096);
const emojiRegex = __nccwpck_require__(3652)();
...
const ansiRegex = __nccwpck_require__(140);
...
const widestLine = __nccwpck_require__(866);
const cliBoxes = __nccwpck_require__(3733);
const camelCase = __nccwpck_require__(1362);
const ansiAlign = __nccwpck_require__(2225);
const wrapAnsi = __nccwpck_require__(9824);
...
const cliBoxes = __nccwpck_require__(7704);
...
const { version } = __nccwpck_require__(2025);
const { tokenize, parse } = __nccwpck_require__(5808);
const suggest = __nccwpck_require__(3828);
const walk = __nccwpck_require__(3477);
const stringify = __nccwpck_require__(8212);
const compile = __nccwpck_require__(8955);
const buildin = __nccwpck_require__(9908);
const methods = __nccwpck_require__(4968);
const createStatApi = __nccwpck_require__(4876);
...
const { naturalCompare, naturalAnalyticalCompare } = __nccwpck_require__(8290);
...
const createError = __nccwpck_require__(7922);
const nodes = __nccwpck_require__(3341).compile;
...
const GetProperty = __nccwpck_require__(3753).build;
const Identifier = __nccwpck_require__(5177).build;
...
const nodes = {
    Arg1: __nccwpck_require__(4534),
    Array: __nccwpck_require__(9188),
    Binary: __nccwpck_require__(9146),
    Block: __nccwpck_require__(678),
    Compare: __nccwpck_require__(9939),
    Conditional: __nccwpck_require__(4992),
    Context: __nccwpck_require__(4102),
    Current: __nccwpck_require__(641),
    Data: __nccwpck_require__(928),
    Declarator: __nccwpck_require__(7557),
    Definition: __nccwpck_require__(7481),
    Filter: __nccwpck_require__(9237),
    Function: __nccwpck_require__(8135),
    GetProperty: __nccwpck_require__(3753),
    Identifier: __nccwpck_require__(5177),
    Literal: __nccwpck_require__(3675),
    Map: __nccwpck_require__(3267),
    MapRecursive: __nccwpck_require__(5929),
    Method: __nccwpck_require__(3418),
    MethodCall: __nccwpck_require__(7621),
    Object: __nccwpck_require__(5950),
    ObjectEntry: __nccwpck_require__(8741),
    Parentheses: __nccwpck_require__(1583),
    Pick: __nccwpck_require__(3007),
    Pipeline: __nccwpck_require__(5297),
    Reference: __nccwpck_require__(6907),
    SliceNotation: __nccwpck_require__(9298),
    SortingFunction: __nccwpck_require__(2214),
    Spread: __nccwpck_require__(4601),
    Template: __nccwpck_require__(5021),
    Unary: __nccwpck_require__(3530)
};
...
};if (true) module.exports = exports = lexer;;
...
if (true) module.exports = exports = parser
return parser;
})()
...
const nodes = __nccwpck_require__(3341).stringify;
...
const nodes = __nccwpck_require__(3341).suggest;
const walk = __nccwpck_require__(3477);
...
const nodes = __nccwpck_require__(3341).walk;
...
const { addToSet, isPlainObject } = __nccwpck_require__(6392);
const contextToType = {
    'path': 'property',
    'key': 'value',
    'value': 'value',
...
// EXTERNAL MODULE: ./node_modules/boxen/index.js
var boxen = __nccwpck_require__(1090);
var boxen_default = /*#__PURE__*/__nccwpck_require__.n(boxen);

// CONCATENATED MODULE: external "child_process"
const external_child_process_namespaceObject = require("child_process");;
// CONCATENATED MODULE: ./typings/enum-types.ts
/**
 * Profile
...
(function (OperationMode) {
    OperationMode["package_versions"] = "package-dependency-versions";
    OperationMode["multi_package_versions"] = "multi-package-dependency-versions";
...
var ErrorType;
(function (ErrorType) {
    ErrorType["general_error"] = "GeneralError";
    ErrorType["parser_error"] = "ParserError";
    ErrorType["validation_error"] = "ValidationError";
    ErrorType["request_error"] = "RequestError";
    ErrorType["response_error"] = "ResponseError";
    ErrorType["parameter_error"] = "ParameterError";
    ErrorType["type_error"] = "TypeError";
    ErrorType["value_error"] = "ValueError";
})(ErrorType || (ErrorType = {}));
//--------------------------------------------------------------------------------------------------

// CONCATENATED MODULE: ./typings/domain-types.ts
...
// CONCATENATED MODULE: ./typings/service-types.ts
...
// CONCATENATED MODULE: ./src/utils/commons.ts


const commons_toString = (value, delim = '') => Array.isArray(value) ? value.join(delim) : value;
const mergeProps = (...obj) => lodash_default().mergeWith({}, ...obj, (o, s) => {
    return lodash_default().isArray(s) && lodash_default().isArray(o) ? lodash_default().union(o, s) : lodash_default().isNull(s) ? o : s;
});
/**
 * Utility function to create a K:V from a list of strings
 * @param values initial input array to operate by
 */
const strToEnum = (values) => {
 ...
 * @param func
 */
const toEnum = (values, func) => {
    return values.reduce((res, key) => {
        res[key] = (func && func(key)) || key;
        return res;
    }, Object.create(null));
};
const isInRange = (actual, min, max) => {
    return actual >= min && actual <= max;
};
const toInt = (str, defaultValue = 0) => {
    try {
...
const isValidFile = (fileName, extension = '') => {
...
// CONCATENATED MODULE: ./src/configs/configs.ts
...
// CONCATENATED MODULE: ./src/utils/profiles.ts




const { NODE_ENV } = process && process.env;
const getProfileByEnv = (env = NODE_ENV) => {
    return env && hasProperty(CONFIG, env) ? CONFIG[env] : CONFIG.dev;
};
const profile = getProfileByEnv();

// CONCATENATED MODULE: ./src/utils/loggers.ts
...
const getColor = (value, defaultValue = '') => (process.stdout.isTTY ? value : defaultValue);
const COLORS = {
    RESET: getColor('\u001B[0m'),
...
const getTime = (format = DATETIME_FORMAT, utc = false) => {
    return dateformat_default()(Date.now(), format, utc);
};
const toLog = (message, ...args) => {
    console.group('>>>');
    console.log(`${COLORS.PURPLE}${getTime()}:${COLORS.RESET}`, message, ...args);
    console.groupEnd();
};
const createLogger = (logger, processor) => {
    return (message, ...args) => {
        logger(processor ? processor(message) : message, ...args);
    };
};
const logs = createLogger((message, ...args) => console.log(`${COLORS.PURPLE}${getTime()}:${COLORS.RESET}`, message, args));
const errorLogs = createLogger((message, ...args) => console.error(`${COLORS.RED}${getTime()}:${COLORS.RESET}`, message, args));
const warnLogs = createLogger((message, ...args) => console.warn(`${COLORS.GREEN}${getTime()}:${COLORS.RESET}`, message, args));
const debugLogs = createLogger((message, ...args) => console.debug(`${COLORS.BLUE}${getTime()}:${COLORS.RESET}`, message, args));
const traceLogs = createLogger((message, ...args) => console.trace(`${COLORS.CYAN}${getTime()}:${COLORS.RESET}`, message, args));
const boxenLogs = createLogger(console.log, message => boxen_default()(message, profile.outputOptions));
const boxenErrorLogs = createLogger(console.error, message => boxen_default()(message, mergeProps(profile.outputOptions, {
    borderColor: 'red',
    borderStyle: 'double',
})));
const boxenWarnLogs = createLogger(console.warn, message => boxen_default()(message, mergeProps(profile.outputOptions, { borderColor: 'green' })));
const boxenDebugLogs = createLogger(console.debug, message => boxen_default()(message, mergeProps(profile.outputOptions, { borderColor: 'blue' })));
const boxenTraceLogs = createLogger(console.trace, message => boxen_default()(message, mergeProps(profile.outputOptions, { borderColor: 'cyan' })));
const coreInfo = createLogger(core.info, message => boxen_default()(message, profile.outputOptions));
const coreError = createLogger(core.error, message => boxen_default()(message.toString(), profile.outputOptions));
const coreDebug = createLogger(core.debug, message => boxen_default()(message.toString(), profile.outputOptions));
const logArrayElements = (index, array) => {
    logs(`array[${index}] = ${array[index]}`);
};

// CONCATENATED MODULE: ./src/utils/serializers.ts
const serialize = (obj, space = 4, callback) => {
    return JSON.stringify(obj, callback, space);
};
const serializers_deserialize = (obj, callback) => {
...
const ensureDirExists = (dir, options = { recursive: true }) => {
    (0,external_fs_.existsSync)(dir) || (0,external_fs_.mkdirSync)(dir, options);
};
const getFilesizeInBytes = (filename) => {
    //return await fs.promises.stat(file)).size
    return statSync(filename).size;
};
const getConfigOptions = (fileName) => {
    const fileData = readFileSync(fileName);
    return deserialize(fileData.toString());
};
const storeDataAsJson = (filePath, fileName, data) => {
    ensureDirExists(filePath);
    const targetPath = (0,external_path_.join)(filePath, fileName);
    coreInfo(`Storing JSON data to target file: ${targetPath}`);
    (0,external_fs_.writeFile)(targetPath, serialize(data), err => {
        if (err) {
            throw err;
        }
    });
};
const files_isFileExists = (fileName, mode = constants.F_OK | constants.R_OK) => {
    try {
...
const getSizeInBytesAsync = async (filename) => {
    return (await promises.stat(filename)).size;
};
const getSizeInBytes = (filename) => {
    return statSync(filename).size;
};
const createWritableStream = (filename) => {
    closeSync(openSync(filename, 'w'));
    return createWriteStream(filename, { flags: 'w' });
};

// CONCATENATED MODULE: ./src/utils/processes.ts




const DEFAULT_EXEC_CALLBACK = (error, stdout, stderr) => {
    error ? errorLogs(stderr) : logs(stdout);
};
const execCommand = (command, options, cb = DEFAULT_EXEC_CALLBACK) => {
    return (0,external_child_process_namespaceObject.exec)(command, options, cb);
};

// CONCATENATED MODULE: ./src/errors/extendable.error.ts
...
// CONCATENATED MODULE: ./src/errors/general.error.ts
...
// CONCATENATED MODULE: ./src/errors/value.error.ts
 ...
 */
class ValueError extends GeneralError {
    /**
...
const valueError = (message, ...args) => {
    return new ValueError(message, args);
};

// CONCATENATED MODULE: ./src/operations/package-versions.operation.ts
...
const execCallback = (options) => (error, stdout, stderr) => {
    if (error) {
        throw valueError(stderr);
    }
    const tree = serializers_deserialize(stdout);
...
const processTree = (pkg, name, map = {}) => {
    const deps = [];
...
// EXTERNAL MODULE: ./node_modules/jora/src/index.js
var src = __nccwpck_require__(378);
// CONCATENATED MODULE: external "util"
const external_util_namespaceObject = require("util");;
// CONCATENATED MODULE: ./src/operations/multi-package-versions.operation.ts
...
async function multiPackageVersionsOperation(options) {
    ...
    try {
        const { command, args } = options.commandOptions;
        const expression = commons_toString([command, ...args], ' ');
        await execCommandAsync(expression, profile.execOptions, multi_package_versions_operation_execCallback(options));
        return Promise.resolve(OperationStatus.success);
    }
    catch (error) {
        errorLogs(error.message);
...
const getRequiredProperty = (property) => {
...
/******/ 	return __nccwpck_require__(2345);
/******/ })()
;